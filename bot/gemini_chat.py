# -*- coding: utf-8 -*-
"""Gemini client — חילוץ תמונות (Vision) בלבד."""
...
def create_vision_client(*, timeout_ms: int | None = None) -> genai.Client:
    api_key = require_env(*GEMINI_VISION_KEY_NAMES, label="Gemini vision API key")
    return _build_client(
        api_key,
        timeout_ms=timeout_ms if timeout_ms is not None else GEMINI_HTTP_TIMEOUT_MS,
    )


def create_gemini_client(*, timeout_ms: int | None = None) -> genai.Client:
    """תאימות לאחור — vision client."""
    return create_vision_client(timeout_ms=timeout_ms)


def invalidate_gemini_config_cache() -> None:
    """איפוס lazy clients (למשל אחרי reload הגדרות)."""
...
def gemini_runtime() -> tuple[genai.Client, str]:
    """לקוח + מודל לחילוץ תמונות."""
    global _vision_client, _vision_model
    if _vision_client is None:
        _vision_client = create_vision_client()
        _vision_model = resolve_primary_model()
    return _vision_client, _vision_model or DEFAULT_MODEL
    ...
    if isinstance(exc, APIError):
        ...
        if exc.code == 400 and "API key not valid" in str(exc):
            return (
                ...
                "עדכן GEMINI_VISION_API_KEY מ-Google AI Studio והפעל מחדש."
            )
