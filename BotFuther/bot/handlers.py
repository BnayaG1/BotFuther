# -*- coding: utf-8 -*-
"""Handlers טלגרם — חילוץ מתמונות + חישוב והשוואת ריאקציות."""
...
from bot.config import BOT_DISPLAY_NAME, VISION_ASYNC_ENABLED
...
from bot.draft_preview import render_solve_preview_temp
...
from bot.solution_check import try_build_student_check_reply
...
async def _reply_text_safe(message, text: str, *, parse_mode: str = "Markdown") -> None:
...
async def _deliver_approved_solve(
    ...
    *,
    ...
    """אחרי אישור טיוטה: שרטוט + טקסט (כמו קודם), ואז תמונת מחברת מלאה."""
    has_result = bool((solved or {}).get("result"))
    preview_path = None
    notebook_path = None
    if has_result:
        preview_path = render_solve_preview_temp(extracted, solved)
            ...
            try:
                with preview_path.open("rb") as photo:
                    await context.bot.send_photo(
                        chat_id=chat_id,
                        photo=photo,
                        caption="📐 קורה + ריאקציות",
                    )
            except Exception as exc:
                log.warning("Failed to send solve preview chat=%s: %s", chat_id, exc)
...
def build_start_welcome_text() -> str:
    return (
        f"היי! 👋\n"
        f"ברוך הבא ל-{BOT_DISPLAY_NAME} – העוזר ההנדסי שלך לסטטיקה.\n\n"
        "אני כאן כדי לעזור לך להבין תרגילים, לעבור על שלבי פתרון "
        "ולסגור פינות כשהחומר לא מובן. 🏗️\n\n"
        "*איך עובדים?*\n\n"
        "שלח תמונה או טקסט: צלם את הקורה או את התרגיל שנתקעת בו.\n\n"
        "קבל פתרון: אקבל ניתוח מפורט של התרגיל, צעד אחרי צעד.\n\n"
        "מיקוד: אני מספק פתרון הנדסי מהיר כדי שלא תתעכב על תרגיל אחד שעות.\n\n"
        "במה נתחיל היום?"
    )
...
def build_start_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("פתרון תרגיל חדש", callback_data="menu:new")],
            [InlineKeyboardButton("הסבר מושג/תקן", callback_data="menu:concept")],
            [InlineKeyboardButton("דיווח על בעיה", callback_data="menu:report")],
        ]
    )
...
_MENU_REPLIES: dict[str, str] = {
    "new": (
        "מעולה — שלח עכשיו תמונה 📸 של התרגיל, או כתוב את הנתונים בטקסט "
        "(אורך קורה, עומסים, סמכים)."
    ),
    "concept": "על איזה מושג או תקן תרצה הסבר? כתוב בטקסט חופשי.",
    "report": (
        "ספר מה לא עבד — נשמח לתקן. אפשר לתאר בטקסט או לצרף צילום מסך."
    ),
}
...
async def cmd_reset(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("שלח תמונה חדשה.")
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    solved = get_stored_vision_solved(chat_id)
    check_reply = try_build_student_check_reply(solved, text)
    if check_reply:
        await _reply_text_safe(update.message, check_reply)
        return

    if is_draft_pending(chat_id) and not looks_like_draft_patch(text):
        await _reply_text_safe(update.message, _TEXT_UNHANDLED)
        return
