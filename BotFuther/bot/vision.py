# -*- coding: utf-8 -*-
"""חילוץ תרגיל מתמונה, חישוב מקומי, והקשר שמור לשאלות המשך."""
...
from bot.config import (
    COG_CATALOG_ALIASES,
    CONVERSATIONAL_FOLLOWUP_HINT,
    DATA_CORRECTION_HINT,
    DEFAULT_MODEL,
    EXPLICIT_CALC_HINT,
    FALLBACK_MODELS,
...
def user_is_correcting(prompt: str) -> bool:
    """האם המשתמש מתקן, מעדכן נתונים או חולק על תשובה קודמת."""
    return bool(USER_CORRECTION_HINT.search(prompt or ""))


def is_data_correction(prompt: str) -> bool:
    """הסטודנט נותן נתונים מתוקנים (במיוחד אחרי שאלת 'מה הטעות')."""
    text = prompt or ""
    return bool(re.search(r"\d", text)) and bool(DATA_CORRECTION_HINT.search(text))


def is_conversational_followup(prompt: str) -> bool:
    """שאלות הסבר/הבנה — לא לחזור על אותה תשובה מספרית."""
    return bool(CONVERSATIONAL_FOLLOWUP_HINT.search(prompt or ""))


def should_attach_vision_brief(prompt: str) -> bool:
    """רקע קצר מתמונה — רק כשבאמת צריך, לא בכל הודעה."""
    from bot.intent import has_engineering_intent, is_small_talk

    text = prompt or ""
    if user_is_correcting(text) or is_conversational_followup(text) or is_small_talk(text):
        return False
    if IMAGE_REFERENCE_HINT.search(text):
        return True
    if not has_engineering_intent(text):
        return False
    return bool(SOLUTION_REQUEST_HINT.search(text) or EXPLICIT_CALC_HINT.search(text))
...
def vision_already_solved(chat_id: int) -> bool:
    bundle = _vision_bundle_by_chat.get(chat_id)
    if not bundle:
        return False
    solved = bundle.get("solved") or {}
    return bool(solved.get("tool_name") and solved.get("result"))


def get_stored_vision_results(chat_id: int) -> str | None:
    """תוצאות חישוב שמורות מתמונה קודמת — לשאלות «ריאקציות» / «פתרון»."""
    ...
    if not tool_name or not isinstance(result, dict):
        return None
    return format_vision_results_only(tool_name, result)


def solve_stored_vision_exercise(chat_id: int) -> dict:
    """מחשב מתרגיל שחולץ ושמור (בלי תמונה חוזרת)."""
    if VISION_EXTRACT_ONLY_MODE:
        raise ValueError(
            "מצב בדיקת חילוץ — חישובים חסומים כרגע."
        )
    extracted = get_stored_vision_extracted(chat_id)
    if not extracted:
        raise ValueError("אין תרגיל שמור מהתמונה — שלח תמונה קודם")
    solved = solve_from_vision_data(extracted)
    store_vision_context(chat_id, extracted, solved)
    return solved


def reply_from_stored_vision_solve(chat_id: int, *, results_only: bool = True) -> str:
    """מחשב (אם צריך) ומחזיר תשובה לבקשת פתרון אחרי חילוץ בלבד."""
    if VISION_EXTRACT_ONLY_MODE:
        extracted = get_stored_vision_extracted(chat_id)
        if extracted:
            return format_vision_extract_only_reply(extracted)
        return ""

    if vision_already_solved(chat_id):
        cached = get_stored_vision_results(chat_id)
        if cached:
            return f"*תוצאות:*\n{cached}" if results_only else cached

    extracted = get_stored_vision_extracted(chat_id)
    if not extracted:
        return ""

    solved = solve_stored_vision_exercise(chat_id)
    if results_only:
        results = format_vision_results_only(
            str(solved.get("tool_name", "")),
            solved.get("result") or {},
        )
        return f"*תוצאות:*\n{results}"
    return format_vision_solve_reply(extracted, solved, "")


def get_vision_brief_context(chat_id: int) -> str | None:
    """סיכום קצר מתמונה — בלי לדבוק שוב את כל התוצאות המספריות."""
    bundle = _vision_bundle_by_chat.get(chat_id)
    if not bundle:
        return None
    extracted = bundle.get("extracted") or {}
    if extracted:
        return "[רקע קצר מתמונה קודמת]\n" + summarize_vision_extraction(extracted)
    reply_text = bundle.get("reply_text")
    if reply_text:
        return "[רקע מתמונה קודמת]\n" + str(reply_text)[:500]
    return None
...
def extract_exercise_with_retries(
    ...
    *,
    ...
    for model in model_order:
        fast_attempts = 1
        if VISION_FAST_MODE and not VISION_FAST_FALLBACK_STAGED:
            fast_attempts = 2
        for attempt in range(fast_attempts):
            ...
            try:
                ...
                last_error = exc
                if VISION_FAST_MODE and attempt == 0 and _is_json_parse_error(exc):
