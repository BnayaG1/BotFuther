# -*- coding: utf-8 -*-
"""משוואת שיווי משקל ΣMb = 0 — מוצאים את Ay (2 סמכים)."""
...
def _mb_product_expr_explain(sign: str, dist_txt: str, force_txt: str) -> str:
    """כמו _mb_product_expr, אך להצגה בהסבר בלבד — הסימן אחרי הסוגריים."""
...
class _MbVerticalTerm:
    ...
    """True על החלק הראשון של מפורס שפוצל סביב נקודת המומנט."""


def _collect_mb_vertical_terms(
    extracted: dict,
) -> tuple[list[_MbVerticalTerm], float, float]:
    ...
    if isinstance(loads, list):
        for ld in loads:
            ...
            if t == "distributed":
                ...
                segs = distributed_moment_segments_about(w_abs, x1, x2, rb_pos)
                split = len(segs) > 1
...
def _term_rotation_about_b_hebrew(
    term: _MbVerticalTerm, rb_pos: float
) -> tuple[str, str]:
    if term.kind == "moment":
...
def _describe_mb_vertical_term_hebrew(
    term: _MbVerticalTerm, rb_pos: float, *, first: bool
) -> str:
    rot, sign = _term_rotation_about_b_hebrew(term, rb_pos)
    ...
    if term.kind == "ay":
        ...
        prod = _mb_product_expr_explain(sign, dist_txt, term.force_txt)
        body = (
...
def build_ay_mb_equation_message_hebrew(extracted: dict, *, prefix: str = "") -> str:
    """הודעה 1: הסבר ספציפי לתרגיל — למה סביב B, ואיזה עומסים נכנסים למשוואה."""
    terms, _ra_pos, rb_pos = _collect_mb_vertical_terms(extracted)
    vertical_terms = [t for t in terms if t.kind not in ("ay", "moment")]
    moment_terms = [t for t in terms if t.kind == "moment"]

    lines: list[str] = []
    head = f"{prefix} " if prefix else ""
    lines.append(f"{head}נמצא את Ay מהמשוואה ΣMB=0 (סביב הנקודה B).")
...
def build_ay_mb_assembled_equation_hebrew(extracted: dict) -> str:
    ...
    known_moment = 0.0
    for term in terms:
        _rot, sign = _term_rotation_about_b_hebrew(term, rb_pos)
